...
def extract_table_19_from_zip(zip_bytes: bytes) -> list[dict]:
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        for csv_name in csv_candidates:
            ...
            for row in reader:
                code = (row.get("Código do Termo") or row.get("CD_PROCEDIMENTO") or
                        row.get("CODIGO") or row.get("cd_procedimento") or
                        row.get("codigo") or "").strip()
                name = (row.get("Termo") or row.get("DS_PROCEDIMENTO") or
                        row.get("DESCRICAO") or row.get("TERMO") or
                        row.get("ds_procedimento") or row.get("descricao") or
                        row.get("termo") or "").strip()
                if not code or not name:
                    continue

                grupo = (row.get("NOME TÉCNICO") or row.get("GRUPO") or
                         row.get("grupo") or "").strip()
                subgrupo = (row.get("SUBGRUPO") or row.get("subgrupo") or "").strip()
                fabricante = (row.get("Fabricante") or row.get("FABRICANTE") or "").strip()
                reg_anvisa = (row.get("Registro Anvisa") or row.get("REGISTRO_ANVISA") or "").strip()
...
def parse_tuss_csv_content(content: str, delimiter: str = ";") -> list[dict]:
    ...
    for row in reader:
        ...
        records.append({
            ...
            "nome": name,
            "display_normalized": normalize_text(name),
            "grupo": (row.get("NOME TÉCNICO") or row.get("GRUPO") or row.get("grupo") or "").strip() or None,
...
def extract_table_22_from_zip(zip_bytes: bytes) -> list[dict]:
    """Extrai CSV da Tabela 22 (Procedimentos) do ZIP e retorna lista de dicts."""
    records = []
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        csv_candidates = [
            n for n in zf.namelist()
            if n.endswith(".csv") and ("22" in n or "procedimento" in n.lower())
        ]
        if not csv_candidates:
            return records
...
async def ingest_tuss_procedures(db_session, records: list[dict], versao: str) -> dict:
    """Upsert de procedimentos TUSS (Tabela 22) no banco. Idempotente."""
    ...
    for rec in records:
        existing = await db_session.execute(
            select(TussProcedure).where(TussProcedure.codigo_tuss == rec["codigo_tuss"])
        )
        existing = existing.scalar_one_or_none()
        if existing:
            existing.nome = rec["nome"]
...
async def run_etl(db_session=None):
    ...
    print("[TUSS ETL] Extraindo Tabela 19 (Materiais)...")
    records = extract_table_19_from_zip(zip_bytes)
    print(f"[TUSS ETL] {len(records)} materiais encontrados")

    print("[TUSS ETL] Extraindo Tabela 22 (Procedimentos)...")
    proc_records = extract_table_22_from_zip(zip_bytes)
    print(f"[TUSS ETL] {len(proc_records)} procedimentos encontrados")

    if db_session:
        versao = last_modified or datetime.utcnow().strftime("%Y%m%d")
        mat_result = await ingest_tuss_materials(db_session, records, versao)
        print(f"[TUSS ETL] Materiais: {mat_result}")

        proc_result = await ingest_tuss_procedures(db_session, proc_records, versao)
        print(f"[TUSS ETL] Procedimentos: {proc_result}")

        return {
            "materiais": mat_result,
            "procedimentos": proc_result,
        }

    return {
        "materiais_parsed": len(records),
        "procedimentos_parsed": len(proc_records),
        "sha256": sha,
    }
